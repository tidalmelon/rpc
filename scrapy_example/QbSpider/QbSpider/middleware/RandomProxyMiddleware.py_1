...
class ProxyMiddleware(object):
    ...
    def process_request(self, request, spider):
        ...
        if int(spider.settings.get("USELOCALIP"))==1:

            request.meta.pop("proxy", None)

        else:

            print 'proxy size: %s' % len(self.proxys)
    ...
    def process_response(self, request, response, spider):
        ...
        if response.status > 300 and response.status in retry_http_code:
            ...
            if int(spider.settings.get("USELOCALIP"))==1:
                ...
                #raise CloseSpider("%s"%response.status)
