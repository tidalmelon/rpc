function reset_txtAccount() {
    ...
    var n = $("#hidUType").val();
    n == "204" ? ($("#txtAccount").val(txtAccount_Default_YWMob).attr("data-preval", ""), $("#txtShowPwd").val(txtShowPwd_Default_YWMob).attr("data-preval", txtShowPwd_Default_YWMob)) : ($("#hidUType").val(""), $("#txtAccount").val(txtAccount_Default).attr("data-preval", ""), $("#txtShowPwd").val(txtShowPwd_Default).attr("data-preval", txtShowPwd_Default));
...
function showCaptcha() {
    var n = webAppPath + "captcha", r = $("#imgCaptcha").attr("data-guid"), t = "100", i = "37";
    n = n + "?" + r + "&source=login&width=" + t + "&height=" + i + "&" + Math.random();
    $("#imgCaptcha").attr({src: n, width: t, height: i}).show()
}
...
function reqInfo_Cookie_D() {
    var n = reqInfo_Cookie_Name;
    $.cookie(n, null, {path: "/"})
}
function sendRandomPwd() {
    $("#liCaptcha").is(":visible") ? showCaptcha() : randomPwd_Cookie_R();
    var i = $("#txtAccount").val(), r = $("#divForgetPwd"), t = $("#divGetRandomPwd"), n = $("#divCxhq");
    $.postJSON(webAppPath + "login/ajax", {m: "sendrandompwd", phone: i}, function (i) {
        var f, e, o, u;
        i != null ? (f = i, e = f.ResultCode, e == "-1" ? showErrMsg(f.Desc) : e == "0" ? (randomPwd_Cookie_W(), o = 60, u = o, rdmPwdInterval = window.setInterval(function () {
            u == o && (showErrMsgOK("随机码发送成功，请注意查收！"), r.hide(), t.hide(), n.show());
            u >= 0 ? (t.attr("data-times", u), n.html('<span class="txtright">' + u + "秒后可重新获取<\/span>"), u--) : (n.html('<a class="greenA">重新获取<\/a>').find("a").unbind("click").bind("click", function () {
                sendRandomPwd()
            }), clearInterval(rdmPwdInterval))
        }, 1e3)) : showErrMsg(f.Desc)) : showErrMsg("系统繁忙，稍后再试")
    })
}
function randomPwd_Cookie_W() {
    var r = randomPwd_Cookie_Name, n = $.cookie(r), t, u, f = "login.189.cn", i, e, o;
    n == undefined ? t = 1 : (n = CryptoJS.AES.decrypt(n, f).toString(CryptoJS.enc.Utf8), i = n.split("$"), e = parseInt(i[0]), o = i[1], t = e + 1);
    u = (new Date).getTime();
    n = t + "$" + u;
    n = CryptoJS.AES.encrypt(n, f);
    $.cookie(r, n, {path: "/"})
}
function randomPwd_Cookie_R() {
    var r = randomPwd_Cookie_Name, n = $.cookie(r), t, i, u;
    n != undefined && (n = CryptoJS.AES.decrypt(n, "login.189.cn").toString(CryptoJS.enc.Utf8), t = n.split("$"), i = parseInt(t[0]), u = t[1], i >= 3 && ($("#liCaptcha").show(), $("#liCaptcha").is(":visible") && ($("#txtPassword").unbind("keypress"), showCaptcha())))
}
function randomPwd_Cookie_D() {
    var n = randomPwd_Cookie_Name;
    $.cookie(n, null, {path: "/"})
}
function loadLoginCaptcha() {
    var t = $("#txtAccount").val(), n = $("#hidUType").val(), i = $("#hidProvinceID").val(), r = $("#hidAreaCode").val(), u = $("#hidCityNo").val();
    t != "" && (t != txtAccount_Default || n == "204") && (t != txtAccount_Default_YWMob || n != "204") && n != "" && (n != "202" && n != "203" || i != "" && r != "" && u != "") && ($.postJSON(webAppPath + "login/ajax", {
        m: "loadlogincaptcha",
        Account: t,
        UType: n,
        ProvinceID: i,
        ...
        if (n != null) {
            var t = n;
            if (t.ResultCode == 0 && (loginFailTimes = t.FailTimes, loginLockTimes = t.LockTimes, t.CaptchaFlag == 1)) {
                if ($("#liCaptcha").is(":visible"))return;
                showCaptcha();
                $("#liCaptcha").show()
            }
        } else showErrMsg("系统繁忙，稍后再试")
    }, !1), set_main_height())
}
function wjmmShow() {
    $("#divMain").hide();
...
var webAppPath = $("html").attr("data-apppath"), loginFormErr = $("#loginForm").attr("data-result") == "1", loginFailTimes = 0, loginLockTimes = 0, txtAccount_Default = "电信手机/固话/宽带/注册邮箱", txtAccount_Default_YWMob = "移动/联通手机号码", txtCityNo_Default = "地市（中文/拼音）", txtShowPwd_Default = "用户密码", txtShowPwd_Default_ = "用户密码", txtShowPwd_Default_Random = "随机密码", txtShowPwd_Default_YWMob = "网站密码", txtCaptcha_Default = "验证码", aRandomPwd_Default = "使用随机密码登录", aRandomPwd_Default_ = "使用用户密码登录", rdmPwdInterval;
$(function () {
    var r, n, u, t, i;
    ...
    $("#aMoreLoginStyle").click(function () {
        $("#divLoginHeader").hide();
        $("#divLoginHeaderMore").show();
        $("#divLoginBox").hide();
        $("#divLoginBoxMore").show();
        set_main_height()
    });
    $("#aCtAccountLogin").click(function () {
        var t = $("#txtAccount").val(), n = $("#hidUType").val();
        $("#aCtOtherAccount").show();
        $("#aMoreLoginStyle").hide();
        $("#aReturnOthLogin").hide();
        $("#divLoginHeader").show();
        $("#divLoginHeaderMore").hide();
        n == "204" && ($("#hidUType").val(""), reset_txtAccount());
        $("#divLoginBox").show();
        $("#divLoginBoxMore").hide();
        set_main_height()
    });
    $("#aReturnOthLogin").click(function () {
        $("#aCtAccountLogin").click()
    });
    $("#txtAccount").focus(function () {
        ...
        var n, i, t;
        if (hideErrMsg(), $("#divAccount_focus").hide(), n = $(this).val(), n = $.trim(n), $(this).val(n), i = $(this).attr("data-preval"), t = $("#hidUType").val(), n == "")return reset_txtAccount(), !1;
        if (n == i && t.length > 0)return !1;
        if ($("#hidRandomFlag").val("0"), t == "204") {
            if (!checkIsCellphone(n))return showErrMsg("请输入正确的手机号码"), !1
        } else $("#txtShowPwd").val(txtShowPwd_Default).attr("data-preval", txtShowPwd_Default), checkIsCellphone(n) ? ($("#hidUType").val("201"), $("#liTelephone").hide(), $("#txtCityNo").val(txtCityNo_Default), $("#hidProvinceID").val(""), $("#hidAreaCode").val(""), $("#hidCityNo").val(""), $.postJSON(webAppPath + "login/ajax", {
            m: "checkphone",
            phone: n
        }, function (t) {
            var r = t, i = !1;
            r != null ? (i = !0, $("#divRandomPwd").show(), $("#hidProvinceID").val(r.ProvinceID)) : (i = !1, $("#divRandomPwd").hide(), $("#hidProvinceID").val(""));
            checkIsCellphoneForCT(n) && i && ($("#aRandomPwd").length == 0 && $("#divRandomPwd").html('<a class="greenA" id="aRandomPwd"><\/a>'), $("#aRandomPwd").html(aRandomPwd_Default).show(), $("#txtShowPwd").val(txtShowPwd_Default_).attr("data-preval", txtShowPwd_Default_), $("#aRandomPwd").unbind("click").bind("click", function () {
                var r;
                hideErrMsg();
                var i = $("#divForgetPwd"), n = $("#divGetRandomPwd"), t = $("#divCxhq"), u = $("#hidRandomFlag").val();
                u == "1" ? ($("#aRandomPwd").html(aRandomPwd_Default), $("#hidRandomFlag").val("0"), $("#txtShowPwd").val(txtShowPwd_Default_).attr("data-preval", txtShowPwd_Default_), i.show(), n.hide(), t.hide()) : ($("#aRandomPwd").html(aRandomPwd_Default_), $("#hidRandomFlag").val("1"), $("#txtShowPwd").val(txtShowPwd_Default_Random).attr("data-preval", txtShowPwd_Default_Random), i.hide(), n.find("a").unbind("click").bind("click", function () {
                    sendRandomPwd()
                }), r = parseInt(n.attr("data-times")), r > 0 ? (n.hide(), t.show()) : (n.show(), t.hide()))
            }))
        }, !1)) : checkIsTelephone(n) ? ($("#divRandomPwd").hide(), $("#liTelephone").fadeIn("slow"), $("#selectType").hide(), $("#selectType ul li").remove(), $("#selectType ul").append('<li value="202"><a>固话<\/a><\/li>').append('<li value="203"><a>宽带<\/a><\/li>'), $("#txtUType").val("固话"), $("#hidUType").val("202")) : checkIsMail(n) ? ($("#divRandomPwd").hide(), $("#liTelephone").hide(), $("#hidUType").val("1")) : n == txtAccount_Default ? $(this).val("") : ($("#divRandomPwd").hide(), $("#liTelephone").fadeIn("slow"), $("#selectType").hide(), $("#selectType ul li").remove(), $("#selectType ul").append('<li value="203"><a>宽带<\/a><\/li>'), $("#txtUType").val("宽带"), $("#hidUType").val("203"));
    ...
    $("#txtCityNo").focus(function () {
        var n = $(this).val();
        n == txtCityNo_Default && $(this).val("");
        $(this).attr("autocomplete", "off").removeClass("noText").addClass("hasText");
        $("#selectType").hide();
        $("#hotCity").show();
        $("#hidProvinceID").val("");
        ...
        var n = $(this).val();
        n == "" ? ($(this).val(txtCityNo_Default).removeClass("hasText").addClass("noText"), $("#scrollDiv ul li").remove(), $("#suggestCity").hide()) : n != txtCityNo_Default && $(this).removeClass("noText").addClass("hasText");
        loadLoginCaptcha()
    }).keyup(function (n) {
        switch (n.keyCode) {
            case 40:
                return $("#suggestCity").is(":visible") && ($(this).blur(), $("#scrollDiv ul").focus()), !1
        }
        var t = $.trim($(this).val());
        t.length == 0 ? $("#suggestCity").is(":visible") && $("#suggestCity").hide() : ($("#hotCity").is(":visible") && $("#hotCity").hide(), $.postJSON(webAppPath + "login/ajax", {
            m: "querycity",
            key: t
        }, function (n) {
            var u;
            $("#scrollDiv ul li").remove();
            var f = /[a-z]+/i, r = "", i = "";
            n != null && (u = $.parseJSON(n), u.length != 0 ? ($.each(u, function (n, u) {
                r = '<li class="ac"><a href="javascript:;" data-pid="' + u.ProvinceID + '" data-areacode="' + u.AreaCode + '" data-cityno="' + u.CityNo + '" data-cityname="' + u.CityName + '"><span class="longAddressRed">' + t + "<\/span>";
                f.test(t) ? (t.length == 1 ? i = u.Pinyin1.substr(t.length) : t == u.Pinyin2.substring(0, t.length) ? i = u.Pinyin2.substr(t.length) : t == u.Pinyin1.substring(0, t.length) && (i = u.Pinyin1.substr(t.length)), i = i + "(" + u.CityName + ")") : i = u.CityName.substr(t.length);
                r = r + i + "<\/a><\/li>";
                $("#scrollDiv ul").append(r);
                $("#scrollDiv ul li").length < 8 ? $("#scrollDiv ul").css({
                    height: "auto",
                    "overflow-y": "hidden"
                }) : $("#scrollDiv ul").css({height: "224px", "overflow-y": "scroll"})
            }), $("#scrollDiv ul li").length > 0 && ($("#scrollDiv ul li").click(function () {
                var n = $(this).children("a"), t = n.attr("data-cityname"), i = n.attr("data-pid"), r = n.attr("data-areacode"), u = n.attr("data-cityno");
                $("#hidProvinceID").val(i);
                $("#hidAreaCode").val(r);
                $("#hidCityNo").val(u);
                $("#suggestCity").hide();
            ...
            }).eq(0).addClass("ac_over"), $("#suggestCity").show())) : (r = '<li class="ac" style="color: #F57421;font-size: 12px;padding: 1px 0px 0px 12px;width: 150px;">对不起，找不到该地市<\/li>', $("#scrollDiv ul").append(r).css({
                height: "auto",
                "overflow-y": "hidden"
            }), $("#suggestCity").show(), $("#scrollDiv ul li").click(function () {
                $("#suggestCity").hide()
            })))
        }))
    }).mouseleave(function () {
    ...
    $("#txtCaptcha").focus(function () {
        ...
        $("#imgCaptcha").attr("src") == "#" && showCaptcha()
    }).blur(function () {
        var n = $(this).val();
    ...
    $("#loginbtn").click(function () {
        var t = $("#txtAccount").val(), n = $("#hidUType").val(), i = $("#txtCityNo").val(), o = $("#txtShowPwd").attr("data-preval"), u = $("#txtPassword").val(), r = $("#txtCaptcha").val(), f, e;
        if (t == "" || t == txtAccount_Default && n != "204" || t == txtAccount_Default_YWMob && n == "204")return $("#txtAccount").focus(), showErrMsg("请输入用户名"), !1;
        if (n != "201")if (n == "202" || n == "203") {
            if (i == "" || i == txtCityNo_Default)return $("#txtCityNo").focus(), showErrMsg("请输入地市"), !1;
            if (i = $("#hidCityNo").val(), i == "")return $("#txtCityNo").focus(), showErrMsg("请重新输入地市"), !1
        } else if (n == "204") {
            if (!checkIsCellphone(t))return showErrMsg("请输入正确的手机号码"), !1
        } else if ($("#txtAccount").blur(), n = $("#hidUType").val(), n == "")return showErrMsg("请重新输入登录信息"), !1;
    ...
    $("#aYWMobLogin").click(function () {
        var t = $("#txtAccount").val(), n = $("#hidUType").val();
        $("#aCtOtherAccount").hide();
        $("#aMoreLoginStyle").hide();
        $("#aReturnOthLogin").show();
        $("#divLoginHeader").show();
        $("#divLoginHeaderMore").hide();
        n != "204" && ($("#hidUType").val("204"), reset_txtAccount());
        $("#divLoginBox").show();
        $("#divLoginBoxMore").hide();
        set_main_height()
    });
    $("#cmcloginbtn,#cucloginbtn").click(function () {
        $("#aYWMobLogin").click()
    });
    ...
    reqInfo_Cookie_R();
    loginFormErr ? (r = $("#hidUType").val(), n = $("#loginForm").attr("data-resultcode"), n == "9100" ? r == "204" ? (u = webAppPath + "reg", $("#loginForm").attr("data-errmsg", "该账户不存在，请先<a href='" + u + "'>注册<\/a>")) : $("#txtAccount").focusEnd() : n == "9103" ? (t = loginLockTimes - loginFailTimes, t > 0 && $("#loginForm").attr("data-errmsg", "您的密码错误！再连续" + t + "次输入错误，账号将被锁！"), $("#txtShowPwd").focus()) : n == "8105" && $("#loginForm").attr({"data-errmsg": '密码过于简单，请通过“<a style="color: #ff8200" onclick="wjmmShow();">忘记密码<\/a>”进行重置。'}), i = $("#txtCityNo").val(), i == "" || i == txtCityNo_Default || $("#txtCityNo").removeClass("noText").addClass("hasText"), $("#hidRandomFlag").val() == "1" ? (showErrMsgOK("随机码发送成功，请注意查收！"), $("#aRandomPwd").html(aRandomPwd_Default_), $("#txtShowPwd").val(txtShowPwd_Default_Random).attr("data-preval", txtShowPwd_Default_Random), $("#divForgetPwd").hide(), $("#divGetRandomPwd").hide(), $("#divCxhq").show().html('<a class="greenA">重新获取<\/a>').find("a").unbind("click").bind("click", function () {
        sendRandomPwd()
    })) : $("#hidRandomFlag").val("0"), showErrMsg()) : $("#hidRandomFlag").val("0");
...
!function () {
    ...
    function h(t, i, r) {
        n(r.background).is("img") ? i.drawImage(r.background, 0, 0, r.size, r.size) : r.background && (i.fillStyle = r.background, i.fillRect(r.left, r.top, r.size, r.size));
        var u = r.mode;
    ...
    function y(t, i, r) {
        ...
        n(r.fill).is("img") ? (i.strokeStyle = "rgba(0,0,0,0.5)", i.lineWidth = 2, i.stroke(), h = i.globalCompositeOperation, i.globalCompositeOperation = "destination-out", i.fill(), i.globalCompositeOperation = h, i.clip(), i.drawImage(r.fill, 0, 0, r.size, r.size), i.restore()) : (i.fillStyle = r.fill, i.fill())
    }
    ...
    function p(t) {
        return n("<img/>").attr("src", r(t)[0].toDataURL("image/png"))
    }
    ...
    f = function () {
        ...
        var n = {MODE_NUMBER: 1, MODE_ALPHA_NUM: 2, MODE_8BIT_BYTE: 4, MODE_KANJI: 8}, f = {
            ...
            return c.flush(), u = "", u += "<img", u += ' src="', u += "data:image/gif;base64,", u += c, u += '"', u += ' width="', u += n, u += '"', u += ' height="', u += t, u += '"', r && (u += ' alt="', u += r, u += '"'), u + "/>"
        };
        return e
    }()
}(), function () {
    ...
    var n = jQuery, t, i, u = function () {
        ...
        n("div.QRCodeContent").empty().qrcode(i).find("img").addClass("QRCodeImg")
    }, r = function () {
        ...
        n("div.QRCodeContent img").attr({src: t});
        n("#hidQRCodeGuid").val("");
        n("#hidQRCodeText").val("");
